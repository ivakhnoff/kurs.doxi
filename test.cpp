...
SUITE(BaseTests) {
    ...
    TEST(EmptyPath) {
        ...
        // Ожидаем исключение при передаче пустого пути
        CHECK_THROW(b.connect(empty_path), crit_err);
    }
    ...
    TEST(InvalidPath) {
        ...
        // Ожидаем исключение при передаче некорректного пути
        CHECK_THROW(b.connect(invalid_path), crit_err);
    }
    ...
    TEST(InvalidFileExtension) {
        ...
        // Ожидаем исключение при передаче файла без расширения
        CHECK_THROW(b.connect(invalid_extension_file), crit_err);
    }
    ...
    TEST(EmptyFile) {
        ...
        // Ожидаем исключение при передаче пустого файла
        CHECK_THROW(b.connect(empty_base_file), crit_err);
    ...
    TEST(InvalidBase) {
        ...
        // Ожидаем исключение при передаче файла с некорректными данными
        CHECK_THROW(b.connect(invalid_base_file), crit_err);
    ...
    TEST(MissingLogin) {
        ...
        // Ожидаем исключение при отсутствии логина
        CHECK_THROW(b.connect(missing_login_file), crit_err);
    ...
    TEST(MissingPassword) {
        ...
        // Ожидаем исключение при отсутствии пароля
        CHECK_THROW(b.connect(missing_password_file), crit_err);
    ...
    TEST(InvalidLineFormat) {
        ...
        // Ожидаем исключение при некорректном формате строки
        CHECK_THROW(b.connect(invalid_format_file), crit_err);
    ...
    TEST(MultipleColonsInLine) {
        ...
        // Ожидаем исключение при наличии нескольких разделителей
        CHECK_THROW(b.connect(multiple_colons_file), crit_err);
    ...
    TEST(MissingUserLogin) {
        ...
        // Ожидаем исключение при отсутствии обязательного логина "user"
        CHECK_THROW(b.connect(invalid_base_file), crit_err);
    ...
    TEST(MissingMultipleRequiredLogins) {
        ...
        // Ожидаем исключение при отсутствии нескольких обязательных логинов
        CHECK_THROW(b.connect(invalid_base_file), crit_err);
